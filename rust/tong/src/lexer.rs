...
pub fn lex(input: &str) -> Result<Vec<Token>> {
    ...
    let find_line_col = |start: usize| -> (usize, usize) {
        ...
        while lo + 1 < hi {
            let mid = (lo + hi) / 2;
            if line_starts[mid] <= start { lo = mid; } else { hi = mid; }
        }
    ...
    if matches!(kind, TokenKind::Whitespace) { continue; }
    if matches!(kind, TokenKind::Error) { return Err(anyhow!("lex error at {}:{} near '{}'", line, col, text)); }
        tokens.push(Token { kind, text, start: span.start, end: span.end, line, col });
    }
    Ok(tokens)
}
