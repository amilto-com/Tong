...
use crate::parser::{Expr, Pattern, Stmt, TypeAnn, BinOp};
...
impl crate::env::Env {
    ...
    pub fn array_assign(&mut self, name: &str, idx: i64, val: Value) -> Result<()> {
        if idx < 0 {
            bail!("negative index")
        }
    ...
    // Very shallow exhaustiveness check: if scrutinee is a constructor value (or Ident referencing a constructor) and match arms enumerate some constructors without wildcard, warn about missing constructors of the same type. Since types are not tracked, we approximate by collecting constructor names used in patterns and comparing against all constructors known globally whose arity matches the arities seen. This is heuristic but useful for early feedback.
